...
(function($, window, document, undefined) {
	...
	var pluginName = 'searchable', defaults = {
		selector : 'tbody tr',
		childSelector : 'td',
		searchField : '#search',
		striped : false,
		...
		onSearchBlur : false,
		clearOnLoad : false
	}, searchActiveCallback = false, searchEmptyCallback = false, searchFocusCallback = false, searchBlurCallback = false;
	...
	Plugin.prototype = {
		...
		updateStriping : function() {
			var that = this, styles = [ 'oddRow', 'evenRow' ], selector = this.settings.selector
					+ ':visible';
		...
		getFuzzyMatcher : function(term) {
			var regexMatcher, pattern = term.split('').reduce(function(a, b) {
				return a + '[^' + b + ']*' + b;
			});
