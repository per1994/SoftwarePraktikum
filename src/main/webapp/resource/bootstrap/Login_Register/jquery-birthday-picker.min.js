$(function(e) {
	var t = {
		number : [ "01", "02", "03", "04", "05", "06", "07", "08", "09", "10",
				"11", "12" ],
		"short" : [ "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug",
				"Sep", "Oct", "Nov", "Dec" ],
	...
	generateBirthdayPicker = function(n, s) {
		var o = e("<fieldset class='birthdayPicker'></fieldset>"), u = e("<select class='birthYear "
				+ s.sizeClass + "' name='birth[year]'></select>"), a = e(" <select class='birthMonth "
				+ s.sizeClass + "' name='birth[month]'></select>"), f = e(" <select class='birthDate "
				+ s.sizeClass + "' name='birth[day]'></select>");
		...
		for (var h = 0; h <= 11; h++) {
			e("<option></option>").attr("value", h + 1).text(
					t[s.monthFormat][h]).appendTo(a)
		}
		for (var h = 1; h <= 31; h++) {
			var p = h < 10 ? "0" + h : h;
		...
		o.on("change",
				function() {
					...
					if (selectedYear > r) {
						...
						while (n < 12) {
							e("<option></option>").attr("value",
									parseInt(n) + 1).text(t[s.monthFormat][n])
									.appendTo(a);
							n++
						}
					}
					var o = f.children(":last").val();
					var l = (new Date(selectedYear, selectedMonth, 0))
							.getDate();
					if (o > l) {
						while (o > l) {
						...
						while (o < l) {
							var c = parseInt(o) + 1;
							var h = c < 10 ? "0" + c : c;
							e("<option></option>").attr("value", c).text(h)
									.appendTo(f);
							o++
						}
					}
					if (selectedYear * selectedMonth * selectedDay != 0) {
						...
						hiddenDate = selectedYear + "-" + selectedMonth + "-"
								+ selectedDay;
